...
小梅的 `@champion` 巧克力蛋糕放在阿赫迈德的皇家图书馆里——打磨完毕，罗莎的评委测试通过，注册为版本 7。但图书馆里的蛋糕喂不了任何人，顾客也不能走进图书馆点一块蛋糕。这时候**帕特尔医生**出场了。

帕特尔医生是镇上的部署工程师，她管着**发射台**——一个服务柜台，把注册过的蛋糕变成可以下单的商品。她从阿赫迈德的图书馆里拿出小梅的 `@champion` 蛋糕，放到一个带 REST API 的柜台后面。医院食堂现在只需要往 `/invocations` 发一个订单，几秒钟就能拿到蛋糕。没人需要知道小梅用的是什么烤箱，因为托马斯的万能转换头——pyfunc——搞定了翻译。PyTorch 烤箱也好，sklearn 烤箱也好，柜台的接口都一样。

然后需求涨了。医院想在本地部署。河对岸的连锁面包店要在 AWS 上用。学区跑的是 Azure。帕特尔医生没有搭三个单独的柜台。发射台的插件系统让她可以把同一个蛋糕部署到不同目的地——本地一条命令，AWS 一条命令，Azure 一条命令。同一个蛋糕，同一个转换头，不同的发射台。

「小梅烤了一个蛋糕，」帕特尔医生说。「托马斯包装了它。阿毕记录了它。阿赫迈德注册了它。罗莎测试了它。我的工作是最后一公里——确保有人能真正下单买到它。」蛋糕一上柜台，每一笔订单都会被侦探老陈追踪，这样小梅就能看到她的作品在真实世界里表现如何。
...
| 组件 | 对应什么 |
|------|---------|
| 模型注册中心页面 | 可以被部署的模型——版本信息、Alias、阶段 |
| AI 网关配置 | 网关路由可以指向已部署模型的端点 |
| `RunPage` Artifact 标签页 | 被部署的那个模型 Artifact |
