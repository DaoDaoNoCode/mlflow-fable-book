...
「小梅，巧克力蛋糕实验，Run 47，用托马斯的 sklearn 转换头，存在莉娜保管库七号架。老王在这个 Run 上记了三个参数、两个指标。」

每一条记录都把蛋糕跟所有东西关联起来——实验、Run、Artifact、Flavor。阿毕的记录簿里，没有东西是孤零零存在的。

大多数记录就这么留在簿子里。几十个 Run 下来，小梅试过的每一个配方都有记录，哪怕做砸了的。但真正好的呢？小梅会调用 `register_model()`，蛋糕就被送到阿赫迈德的皇家图书馆，变成一个正式的**模型版本**——有版本号，可以打上 `@champion` 这样的 Alias。图书馆的版本通过 `model_id` 链回阿毕的记录簿，血统关系永远断不了。
