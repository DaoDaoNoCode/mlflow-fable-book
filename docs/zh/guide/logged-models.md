...
大多数记录就留在簿子里——横跨几十个 Run，记录着小梅试过的每一个配方，即使那些蛋糕不值得提拔。但真正好的呢？小梅会调用 `register_model()`，蛋糕就被送到楼上阿赫迈德的皇家图书馆，变成一个**模型版本**——正式的目录条目，有自己的版本号，可以打上 `@champion` 这样的 Alias。模型版本通过 `model_id` 链接回阿毕的记录簿条目，这样完整的血统关系永远不会断。

「区别在这里，」一天早上阿毕跟小莲解释。「我的记录簿条目是小梅一做完蛋糕就自动创建的——每个蛋糕都有，好坏都一样。阿赫迈德图书馆的条目是刻意的提拔——有人决定*这个*蛋糕值得成为正式作品。我的记录簿是蛋糕出生的地方，阿赫迈德的图书馆是蛋糕成为正式作品的地方。但没有出生证明，什么都进不去。」
