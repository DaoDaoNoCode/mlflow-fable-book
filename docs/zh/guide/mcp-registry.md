...
**老方**在镇子边上开了间工具棚。小梅烤蛋糕、老陈查 Trace——用的工具不少来自老方：网页爬虫、数据库连接器、搜索 API。老方的东西口碑好，半个镇的 AI 助手都在用。

老方把网页搜索工具从 v1 升到 v2。镇上一半 Agent 立刻跟着换了。

可 v2 有个 bug 没测出来：碰到非英文字符，搜索结果的 Unicode 编码全乱了。一周之内，小梅三个助手返回的原料推荐变成了乱码。罗德里戈的网关把请求路由到这个坏工具上，自己都不知道。没人分得清哪些 Agent 还在用 v1，哪些切了 v2，还有些团队改过自己的副本——那些安全吗？没人说得上来。

这下好了。

危机过后，镇上建了**工具棚登记处**。每件工具注册名称、版本、状态。版本提交之后**不可修改**——想改就出新版本。生命周期四个阶段：Draft、Active、Deprecated、Deleted。老方的 v2 标成 Deprecated，v3 标成 Active。

登记处引入了 Alias——`@production` 指向 v3，所有引用 `@production` 的 Agent 自动拿到正确版本。Access Binding 控制每件工具能从哪些端点访问——审批过的才能连。

老陈追踪请求的时候，Trace 关联到登记处里确切的版本号——再也不会搞混用的是哪个副本。

「我能做出更好的 v2，」老方承认。「但我做不出更好的分发系统。登记处就是干这个的。」
