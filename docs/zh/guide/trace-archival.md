...
运行了六个月之后，侦探老陈遇到了一个问题。他的档案柜——存放每条 Trace 和 Span 的数据库——快要溢出来了。小梅的实验、罗德里戈的网关路由、罗莎的评估——成千上万条 Trace。每条 Trace 里面是一棵 Span 树，有些 Span 带着好几兆的 LLM 输入输出。档案柜又贵，搜索也一周比一周慢。

老陈提出搞**冷库**。最近的 Trace——30 天以内的——留在档案柜里，取用快。这些是热门案件，每天都有人查。更老的 Trace 打包搬到城对面的仓库——对象存储，又大又便宜，就是取的时候稍慢一点。关键在于：每条 Trace 的目录卡片始终留在档案柜里。卡片上记着 Trace ID、时间戳、状态、Tag，以及 Span 级别的关键信息比如类型和耗时。只有大块的 Span 内容——原始的输入输出——搬到了仓库。

这个系统是透明的。搜索 Trace 的时候，不管数据在热的还是冷的地方，查的是同一套目录。老陈在归档的 Trace 上依然可以按 Span 类型、状态或耗时过滤，因为那些列从来没离开档案柜。唯一做不到的是在 Span 的原始内容里搜索——那些文本已经在仓库里了，不在数据库里。

小梅的实验可以设自己的保留策略——比如保留 90 天而不是默认的 30 天，因为她的团队更经常回看旧的 Run。小普的 Workspace 也可以覆盖服务器默认值。每天晚上，一个安安静静的后台任务扫一遍档案柜，检查日期，把过期的搬到仓库。不需要人操心。

「案件不会消失，」老陈跟小梅说。「只是搬到了一个更便宜的架子上。而目录永远不会忘记它们去了哪里。」
